...
// Same source of truth as server.js's BUSINESS.name; requiring server.js
// from here would be circular.
const BRAND = process.env.BUSINESS_NAME || 'ehealthwatch';

function drawHeader(doc, title, subtitle) {
  doc.rect(0, 0, doc.page.width, 86).fill(DARK);
  doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(20).text(BRAND, X, 26);
...
function buildReceiptPdf({ orderId, profileName, amountInr, paymentId, paidAt, consumerName }) {
  ...
  drawHeader(doc, 'Payment Receipt', orderNumber);
  ...
  labelValueRow(doc, 'Order number', orderNumber);
  ...
  line(doc, `${BRAND} — this is a computer-generated receipt.`, { color: FAINT, size: 8.5, gapAfter: 0 });
